...
interface LoginApiResponse {
  ...
  message?: string;
  user: User;
  // token?: string; // Uncomment if your API returns a token
}
...
export class AuthService {
  ...
  login(username: string, password: string): Observable<boolean> {
    ...
    const loginUrl = 'http://localhost:5057/api/auth/login';
    ...
    return new Observable(observer => {
      this.http.post<LoginApiResponse>(loginUrl, body).subscribe({
        next: (response) => {
  ...
  getUsers(): Observable<User[]> {
    const currentUser = this.getCurrentUser();
    if (!currentUser) return new Observable(obs => {
      obs.next([]);
      obs.complete();
    });
